...
const TaskCard = ({ task, onEdit, onDelete, onStatusChange }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/tasks/${task._id}`, {
        method: "DELETE",
        headers: {
  ...
  const handleStatusChange = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token")
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/tasks/${task._id}/status`, {
        method: "PUT",
        headers: {
